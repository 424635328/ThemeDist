...
import { i as isReady, h as zrevrange, g as get } from './redis_EXBth6OG.mjs';
...
function getAllOmniThemes() {
  ...
  if (OmniConfig.crazyThursday) {
    const crazy = OmniConfig.crazyThursday;
...
function communityToComposed(ct) {
  return {
    preset: `community-${ct.id}`,
    presetName: ct.name,
    cssVars: ct.cssVars,
    customCss: ct.customCss || void 0
  };
}
const COMMUNITY_NEWEST_KEY = "td:themes:by_newest";
async function getCommunityThemes(limit = 50) {
  try {
    if (!isReady() || limit <= 0) return [];
    const ids = await zrevrange(COMMUNITY_NEWEST_KEY, 0, limit - 1);
    if (!ids || ids.length === 0) return [];
    const themes = [];
    for (const id of ids) {
      try {
        const ct = await get(`td:theme:${id}`);
        if (ct) {
...
export { getCommunityThemes as a, getOmniDailyTheme as b, getAllOmniThemes as g };
